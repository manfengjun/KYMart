...
@property (nonatomic,strong) NSArray * roadDataSouce;
...
- (void)setUp{
    ...
    UILabel * titleLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 0, self.frame.size.width, kHYTopViewHeight)];
...
- (NSInteger)tableView:(UITableView *)tableView numberOfRowsInSection:(NSInteger)section{
    ...
    if([self.tableViews indexOfObject:tableView] == 0){
        ...
        return self.roadDataSouce.count;
    }
    return self.dataSouce.count;
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    //县级别
        ...
        item = self.roadDataSouce[indexPath.row];
    }
    cell.item = item;
    return cell;
}
...
- (NSIndexPath *)tableView:(UITableView *)tableView willSelectRowAtIndexPath:(NSIndexPath *)indexPath{
    ...
    if([self.tableViews indexOfObject:tableView] == 0){
        ...
        if ([indexPath0 compare:indexPath] != NSOrderedSame && indexPath0) {
            ...
            [self addTopBarItem];
            [self addTableView];
            [self scrollToNextItem:provinceItem];
            return indexPath;
        ...
        if ([indexPath0 compare:indexPath] != NSOrderedSame && indexPath0) {
            ...
            [self addTopBarItem];
            [self addTableView];
            [self scrollToNextItem:cityItem];
            return indexPath;
        ...
        if ([indexPath0 compare:indexPath] != NSOrderedSame && indexPath0) {
            ...
            [self addTopBarItem];
            [self addTableView];
            [self scrollToNextItem:roadItem];
            return indexPath;
...
- (void)setUpAddress:(AddressItem *)item{
    ...
    for (UIButton * btn  in self.topTabbarItems) {
        switch ([item.level intValue]) {
            case 1:
                ...
                break;
            case 2:
                [self.params setObject:item.id forKey:@"city"];
                break;
            case 3:
                [self.params setObject:item.id forKey:@"district"];
                break;
            case 4:
                [self.params setObject:item.id forKey:@"twon"];
                break;
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(0.5 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        if (self.chooseFinish) {
            self.chooseFinish(self.params,self.address);
        }
    });
}
...
- (void)scrollToNextItem:(AddressItem *)item{
    switch ([item.level intValue]) {
        case 1:
            [self.params setObject:item.id forKey:@"province"];
            break;
...
//    [self addTableView];
//
//    self.cityDataSouce = [[CitiesDataTool sharedManager] queryDataWith:@"2" parent_id:cityItem.id];
//    self.districtDataSouce = [[CitiesDataTool sharedManager] queryDataWith:@"3" parent_id:cityItem.id];
//    self.roadDataSouce = [[CitiesDataTool sharedManager] queryDataWith:@"4" parent_id:cityItem.id];
//
//    
...
//        }
//    }
//    for (int i = 0; i <self.roadDataSouce.count; i++) {
//        AddressItem * item = self.roadDataSouce[i];
//        if ([item.name isEqualToString:districtName]) {
//            NSIndexPath * indexPath = [NSIndexPath indexPathForRow:i inSection:0];
